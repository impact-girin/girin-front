  ...
  return (
    <>
      ...
      <Flex
        ...
        padding={"0 10px"}
        style={{ boxShadow: "0 3px 0 1px lightgray" }}
      >
        <Box
          onClick={(e) => setMode("mt_info")}
          padding={"10px"}
          style={{
            borderBottom: mode === "mt_info" ? "3px solid #2DD790" : "none",
          }}
        >
          <Text
            textAlign={"center"}
            fontWeight={"bold"}
            color={mode === "mt_info" ? "#2DD790" : "black"}
          >
            산정보
          </Text>
        </Box>
        <Box
          onClick={(e) => setMode("visit_list")}
          padding={"10px"}
          style={{
            borderBottom: mode === "visit_list" ? "3px solid #2DD790" : "none",
          }}
        >
          <Text
            textAlign={"center"}
            fontWeight={"bold"}
            color={mode === "visit_list" ? "#2DD790" : "black"}
          >
            방명록
          </Text>
        </Box>
        <Box
          onClick={(e) => setMode("food")}
          padding={"10px"}
          style={{
            borderBottom: mode === "food" ? "3px solid #2DD790" : "none",
          }}
        >
          <Text
            textAlign={"center"}
            fontWeight={"bold"}
            color={mode === "food" ? "#2DD790" : "black"}
          >
            식&nbsp;&nbsp;&nbsp;당
          </Text>
        </Box>
      </Flex>
      {mode === "visit_list" && <Box></Box>}
      {mode === "food" && (
        <Box>
          {food_list?.map((i, n) => (
            <ListAtom
